...
// DIALOG POP
jQuery('.add-new-h2').on( 'click' , function() {
...
// DIALOG KEYWORD ADD
jQuery('#wp-rankie-keywords-add-btn').on('click' , function() {
	"use strict";
	...
	var  link=wpRankieSite.replace('http://','');
...
jQuery('#wp-rankie-keywords-change-btn').on('click' , function(){
	...
	jQuery('.wp-rankie-keyword-id:checked').each(function(){
	   ...
	   var changeIndex=oTable.fnGetPosition( jQuery(this).parent().parent()[0] );
	...
	jQuery('.wp-rankie-keyword-id').prop('checked', false); 
...
jQuery('.subsubsub li a').on('click' ,  function() {
...
function  wpRankieUpdateClick(){
	...
	jQuery(nNodes).find('.wp-rankie-update-row').on('click',  function() {
...
//tabs clicker
jQuery('.category-tabs li').on('click', function(){
...
jQuery(document).ready(function() {
	...
	//SELECT 
	 jQuery('#cb-select-all-1').on('click' ,function(){
	     ...
	     if (jQuery(this).prop('checked') ==  true) {
	             ...
	             jQuery('.wp-rankie-keyword-id:visible').prop('checked',  true);   
	...
	jQuery('#rankie-keywords_wrapper').on('click','.rankie-keyword-text', function(){
	    ...
	    jQuery.ajax({
			            ...
			            success: function(data){
			                  ...
			                  jQuery('#rakie-chart tbody ').empty();
			                  jQuery(res[1]).each(function(index,val){
				                	  if (val[0] != 'Rank'){
				                		  jQuery('#rakie-chart tbody ').append( '<tr><td>' + val[0] + '</td> <td>' + val[1] + '</td> <td><a href="' + val[2] + '">' + val[2] + '</a></td> </tr>' ) ;
				                	  }
			                  });
