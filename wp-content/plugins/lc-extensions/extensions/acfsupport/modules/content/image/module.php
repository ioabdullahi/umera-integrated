 ...
 * Module Class
 */
class ACF_Image extends DSLC_Module {
	...
	function __construct() {
		...
		$this->module_id = 'ACF_Image';
		$this->module_title = __( 'Image', 'lc-acf-integration' );
	...
	function options() {
		...
		if ( ! empty( $id ) ) {
			...
			$fields = lcacf_get_all_fields( $id, 'image' );
			...
			$fields = lcacf_get_all_fields_by_group( 'image' );
		...
		$dslc_options = array(
			...
			array(
				'label' => __( 'Link Type', 'lc-acf-integration' ),
				'id' => 'link_type',
				'std' => 'none',
				'type' => 'select',
				'choices' => array(
					...
					array(
						'label' => __( 'URL - Same Tab', 'lc-acf-integration' ),
						'value' => 'url_same',
					),
					array(
						'label' => __( 'URL - New Tab', 'lc-acf-integration' ),
						'value' => 'url_new',
					),
				),
			),
			array(
				'label' => __( 'Link - URL', 'lc-acf-integration' ),
				'id' => 'link_url',
				'std' => '',
				'type' => 'text',
			),
			...
			array(
				...
				'affect_on_change_el' => '.lc-acf-module-image-container',
	...
	function output( $options ) {
		...
		<div class="lc-acf-module-image-container">
			<div class="lc-acf-module-image">
			<?php
 ...
 * Register Module
 */
add_action( 'dslc_hook_register_modules', 'lcacf_init_image' );

function lcacf_init_image() {
    return dslc_register_module( 'ACF_Image' );
}
