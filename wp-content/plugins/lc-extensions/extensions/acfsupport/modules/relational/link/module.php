 ...
 * Module Class
 */
class ACF_Link extends DSLC_Module {
	...
	function __construct() {
		...
		$this->module_id = 'ACF_Link';
		$this->module_title = __( 'Link', 'lc-acf-integration' );
		$this->module_icon = 'anchor';
	...
	function options() {
		...
		if ( ! empty( $id ) ) {
			...
			$fields = lcacf_get_all_fields( $id, 'link' );
			...
			$fields = lcacf_get_all_fields_by_group( 'link' );
		...
		$dslc_options = array(
			...
			array(
				...
				'affect_on_change_el' => '.lc-acf-module-link',
			...
			array(
				...
				array(
					...
					'type' => 'color',
					'refresh_on_change' => false,
					'affect_on_change_el' => '.lc-acf-module-link a',
				...
				array(
					...
					'type' => 'color',
					'refresh_on_change' => false,
					'affect_on_change_el' => '.lc-acf-module-link a:hover',
					'affect_on_change_rule' => 'color',
					'section' => 'styling',
					'tab' => __( 'Typography', 'lc-acf-integration' ),
				),
 ...
 * Register Module
 */
add_action( 'dslc_hook_register_modules', 'lcacf_init_link' );

function lcacf_init_link() {
    return dslc_register_module( 'ACF_Link' );
}
