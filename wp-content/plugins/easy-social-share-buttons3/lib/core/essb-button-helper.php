...
{
	public static function draw_share_buttons(
		$share = array(),
		$style = array(),
		$networks = array(),
		...
		return essb_draw_share_buttons(
			$share,
			$style,
			$networks,
			$networks_order,
...
function essb_draw_share_buttons(
	$share = array(),
	$style = array(),
	$networks = array(),
	$networks_order = array(),
	...
	/**
	 * @since 10.3 Disable share counters
	 */
	...
	if ($share_on_prefix == '') {
		$share_on_prefix = esc_html__('Share on', 'essb');
	}
	...
	foreach ($networks_order as $single) {
		...
		if (in_array($single, $networks)) {
			 ...
			 */
			/**
			 * @since 10.2 - adding deprecated networks check
			 */
			if (ESSB_Share_Networks_Helper::is_deprecated($single)) {
				continue;
			}
			...
			if ($single == 'pinterest' && !essb_is_pinterest_sniff_disabled()) {
			...
			$content .= sprintf(
				'<li class="essb_item essb_link_%8$s nolightbox%2$s">
	...
	if (($is_active_more_button && $style['more_button_func'] == '1')) {
		...
		$content .= sprintf(
			'<li class="essb_item essb_link_%1$s nolightbox%2$s">
	...
	if (is_array($native_buttons) && $native_buttons['active'] && $native_buttons['sameline']) {
		$content .= ESSBNativeButtonsHelper::draw_native_buttons(
			$native_buttons,
			$native_buttons['order'],
			$native_buttons['counters'],
			$native_buttons['sameline'],
			$native_buttons['skinned']
		);
	}
...
function essb_get_share_address($network, $share = array(), $salt = '', $amp_endpoint = false, $is_mobile = false)
{
	...
	switch ($network) {
		...
		case 'twitter':
			...
			$url = sprintf('https://x.com/intent/post?text=%1$s&amp;url=%2$s%3$s%4$s', $use_tweet, $share['short_url_twitter'], $twitter_pass_user, $twitter_pass_hastags);
			break;
		case 'google':
		...
		default:
			// @since 10.2.1
			if (essb_manage_share_networks()->exist_network($network)) {
				$url = essb_manage_share_networks()->get_share_address($network, $share, essb_get_additional_network_share_options($network, $share));
			}
...
function essb_draw_buttons_start($style = array(), $position = '', $salt = '', $like_or_share = 'share', $share = array(), $cached_counters = array(), $networks = array())
{
	...
	/**
	 * @since 9.9 Inline size code
	 */
	$button_size = isset($style['button_size']) && !empty($style['button_size']) ? $style['button_size'] : '';
	$button_size_mobile = isset($style['button_size_mobile']) && !empty($style['button_size_mobile']) ? $style['button_size_mobile'] : '';

	if (!empty($button_size) || !empty($button_size_mobile)) {
		if (!class_exists('ESSB_Share_Button_Size_Styles')) {
			include_once(ESSB3_CLASS_PATH . 'assets/class-share-button-size-styles.php');
		}

		$links_start .= ESSB_Share_Button_Size_Styles::generate_size_code_inline($button_size, $button_size_mobile);

		if (!empty($button_size_mobile)) {
			$css_class_width .= ' essb_size_' . $button_size_mobile . '_mobile';
		}
	}

	/** @since 10.2 - Optimized total counter stylesheets loading  */
	if ($style['show_counter']) {
		$links_start .= ESSB_Total_Counter_Generator::register_additional_styles($style['total_counter_pos'], $position, $salt);
	}

	$links_start .= sprintf(
		'<div class="essb_links%1$s%2$s essb_displayed_%3$s%4$s%5$s essb_%6$s%13$s%14$s%15$s%16$s print-no"
	...
	if ($style['total_counter_pos'] == 'before') {
		$links_start .= ESSB_Total_Counter_Generator::generate($style['total_counter_pos'], $original_cached_total_counter, 'div');
	}
	...
	if ($like_or_share == 'share' && $style['show_counter']) {
		if (
			$style['total_counter_pos'] == 'left' || $style['total_counter_pos'] == 'leftbig'
			|| $style['total_counter_pos'] == 'hidden' || $style['total_counter_pos'] == 'leftbigicon'
		) {
			$links_start .= ESSB_Total_Counter_Generator::generate($style['total_counter_pos'], $original_cached_total_counter);
		}
	}
...
function essb_draw_buttons_end($style = array(), $position = '', $salt = '', $like_or_share = 'share', $native_buttons = array(), $cached_counters = array(), $share = array())
{
	...
	if ($like_or_share == 'share') {
		...
		if ($style['show_counter']) {
			if (
				$style['total_counter_pos'] == 'right' || $style['total_counter_pos'] == 'rightbig'
				|| $style['total_counter_pos'] == 'rightbigicon'
			) {
				$links_start .= ESSB_Total_Counter_Generator::generate($style['total_counter_pos'], $original_cached_total_counter);
			}
		}
