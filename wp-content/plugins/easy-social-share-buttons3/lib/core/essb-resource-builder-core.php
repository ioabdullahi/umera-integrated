...
add_filter('essb_css_buffer_head', 'essb_css_generate_code_button_sizes');

function essb_css_generate_code_button_sizes($buffer) {
    
    if (essb_option_bool_value('pinterest_images') && !essb_is_module_deactivated_on('pinpro')) {
        $pin_button_size = essb_sanitize_option_value('pinterest_button_size');
        $pin_button_size_mobile = essb_sanitize_option_value('pinterest_button_size_mobile');
        
        if (!empty($pin_button_size) || !empty($pin_button_size_mobile)) {
            if (!class_exists('ESSB_Share_Button_Size_Styles')) {
                include_once (ESSB3_CLASS_PATH . 'assets/class-share-button-size-styles.php');
            }
            
            $buffer .= ESSB_Share_Button_Size_Styles::generate_size_code_inline($pin_button_size, $pin_button_size_mobile);
        }
    }    
    return $buffer;
}
...
function essb_js_build_admin_ajax_access_code($buffer) {
	 ...
	 */
	if (essb_option_bool_value('pinterest_images') || essb_is_pinterest_sniff_disabled()) {
	...
	if (essb_option_bool_value('pinterest_images') && !essb_is_module_deactivated_on('pinpro')) {
		...
		$pin_options['disable_mobile'] = essb_option_bool_value('pinterest_disable_mobile');
		...
		/**
		 * @since 10.1 Integration with Pinterest Pro Tools
		 */
		if (essb_option_bool_value('pinpro_advanced_disable')) {
		    if (empty($pin_options['min_width'])) {
		        $pin_options['min_width'] = essb_sanitize_option_value('pinpro_advanced_minwidth');
		    }

		    if (empty($pin_options['min_height'])) {
		        $pin_options['min_height'] = essb_sanitize_option_value('pinpro_advanced_minheight');
		    }
		    
		    if (empty($pin_options['hideon'])) {
		        $pin_options['hideon'] = essb_sanitize_option_value('pinpro_advanced_hideon');
		    }		    
		}
