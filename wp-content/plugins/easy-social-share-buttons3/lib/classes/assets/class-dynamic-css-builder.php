...
class ESSB_Dynamic_CSS_Builder
{
    ...
    public static function is_optimized_loading()
    {
        $r = false;

        if (essb_option_bool_value('precompiled_resources') && essb_option_value('precompiled_mode') != 'js') {
            $r = true;
        }
    ...
    public static function register_dynamic_assets($buffer)
    {
        ...
        if (
            essb_option_bool_value('activate_mailchimp_customizer') ||
    ...
    /**
     * Outputs inline CSS code with an optional style tag wrapper.
     *
     * @param string $key       A unique key to identify the inline code block.
     * @param string $code      The CSS code to be output inline.
     * @param bool   $style_tag Whether to wrap the CSS code in a <style> tag. Default is false.
     *
     * @return string The generated CSS code, either wrapped in a <style> tag or not.
     *               Returns an empty string if optimized loading is enabled.
     */
    public static function output_inline_code($key = '', $code = '', $style_tag = false)
    {
        if (!empty($code)) {

            if (!self::is_optimized_loading()) {
                $r = '';
                if ($style_tag) {
                    $r .= '<style type="text/css" data-key="' . esc_attr($key) . '">';
                }
                $r .= self::minify_advanced($code);
                if ($style_tag) {
                    $r .= '</style>';
                }
                return $r;
            } else {
                self::register_dynamic_code($key, $code);
                return '';
            }
        } else {
            return '';
