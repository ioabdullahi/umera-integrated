...
function essb_register_settings_metabox_optimize()
{
	...
	if (isset($_GET['action'])) {
		...
		$sidebar_options[] = array(
			'field_id' => 'twittertag',
			'title' => esc_html__('Custom X Message', 'essb') . (essb_options_bool_value('twitter_card') ? ' & Card Data' : ''),
			'icon' => 'twitter',
			'type' => 'menu_item',
			'action' => 'default',
			'default_child' => ''
		...
		ESSBOptionsFramework::draw_help('', '', '', array('buttons' => array(
			'How to Customize The Shared Information On X (Custom Tweet)' => 'https://docs.socialsharingplugin.com/knowledgebase/how-to-customize-the-shared-information-on-twitter-custom-tweet/',
		...
		ESSBOptionsFramework::draw_options_row_start(esc_html__('X Message', 'essb'), esc_html__('The default X sharing message will be automatically generating using the post/page title. Using this field you can customize it.', 'essb'));
		ESSBOptionsFramework::draw_textarea_field('essb_post_twitter_tweet', 'essb_metabox', $essb_post_twitter_tweet, $post->post_title);
		ESSBOptionsFramework::draw_options_row_end();

		ESSBOptionsFramework::draw_options_row_start(esc_html__('Set a personalized list of hashtags for this post/page', 'essb'), esc_html__('Enter delimited with a comma. Example: demotag1,demotag2,demotag3', 'essb'));
		...
		ESSBOptionsFramework::draw_options_row_start(esc_html__('Overwrite default X username to be mentioned in the message', 'essb'), '');
		...
		if (essb_is_pinterest_sniff_disabled()) {
			$pin_mode_title = esc_html__('Your Pinterest mode: Pin selected image only (or featured image if there is no selected image)', 'essb');
			$pin_mode_desc = esc_html__('In this mode, the Pinterest button will Pin only the selected image. When there is no custom-selected image the button will use the featured image.    You can change the button mode from Sharing -> Global Network Options -> Pinterest.', 'essb');
		} else {
			$pin_mode_title = esc_html__('Your Pinterest mode: Pin any image from the content', 'essb');
			$pin_mode_desc = esc_html__('In this mode when you press the Pinterest share dialog a new window will appear to select an image for Pin from those available in the content. There is another mode where you can specify a customized image only. You can change the button mode from Sharing -> Global Network Options -> Pinterest.', 'essb');
		}

		ESSBOptionsFramework::draw_options_row_start_full('inner-row-small');
		ESSBOptionsFramework::draw_hint($pin_mode_title, $pin_mode_desc, '', 'glowhelp');
		ESSBOptionsFramework::draw_options_row_end();

		ESSBOptionsFramework::draw_options_row_start(esc_html__('Pinterest Description', 'essb'), esc_html__('Add custom Pinterest description. It will be sent to your pin create command and it will also take part in the Pinterest-specific attribute data-pin-description.', 'essb'));
		...
		/**
		 * @since 10.1 Pinterest hidden images
		 */
		...
		ESSBOptionsFramework::draw_options_row_start(esc_html__('Pinterest Hidden Images', 'essb'), '');
		echo '<a class="button button-secondary" data-post-id="' . esc_attr($post->ID) . '" href="#" id="essb-pinterest-hidden-image-picker">Add Image</a>';
		echo '<div id="essb-pinterest-hidden-image-list">';

		if (!class_exists('ESSB_Post_Meta')) {
			include_once(ESSB3_PLUGIN_ROOT . 'lib/classes/class-post-meta.php');
		}

		$essb_pinterest_hidden = essb_get_post_meta($post->ID, 'pinterest_hidden');

		if (!empty($essb_pinterest_hidden)) {
			$essb_pinterest_hidden = unserialize($essb_pinterest_hidden);

			foreach ($essb_pinterest_hidden as $image) {
				echo '<div class="essb-single-pinterest-hidden-image">';
				echo '<img src="' . $image . '" data-url="' . $image . '" />';
				echo '<span class="essb-single-pinterest-hidden-image-remove"><span class="dashicons dashicons-no-alt"></span></span>';
				echo '<input type="hidden" name="essb_pinterest_hidden[]" value="' . $image . '"/>';
				echo '</div>';
			}
		}
					...
					<div
						...
						<div class="essb-stats-panel-inner">
							<div class="essb-stats-panel-text">
								<?php echo essb_svg_replace_font_icon($key); ?><span
