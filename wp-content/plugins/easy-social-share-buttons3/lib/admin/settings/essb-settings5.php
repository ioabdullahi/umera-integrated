...
<div class="essb-admin-panel essb-ui">

	<?php
	$drawing_tab = $current_tab;

	if ($drawing_tab == 'update' || $drawing_tab == 'status') {
		$drawing_tab = 'about';
	}
	...
	<div class="wrap essb-settings-wrap essb-wrap-<?php echo $drawing_tab; ?>">
		...
		if (
			$current_tab != 'analytics' && $current_tab != 'shortcode' && $current_tab != 'status' && $current_tab != 'welcome' &&
	...
	<div class="essb-helper-popup essb-ui" id="essb-testpopup" data-width="auto" data-height="auto">
	...
	<div class="essb-helper-popup essb-ui" id="essb-networkselect" data-width="800" data-height="auto">
		<div class="essb-helper-popup-title">
			Social Networks Selection
			<a href="#" class="essb-helper-popup-close"></a>
		</div>
...
function essb_settings5_status_notifications()
{
	...
	if ($general_precompiled_resources) {
		...
		ESSBOptionsFramework::draw_card_hint(
			esc_html__('Combine CSS and Javascript files (Pre-compiled Mode)', 'essb'),
			'',
			'internal_cache',
			ESSBOptionsFramework::generate_ui_badge('Running', 'sm', 'primary essb-ui-badge-outline'),
			'<a href="'.$cache_clear_address.'" class="essb-ui-btn essb-ui-btn-sm essb-ui-btn-primary">CLEAR CACHE</a>',
			''
		);
	}
