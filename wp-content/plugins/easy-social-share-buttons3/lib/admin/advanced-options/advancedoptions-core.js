jQuery(document).ready(function ($) {
	...
	essbAdvancedOptions.load = function (content) {
		...
		$('.features-manage .essb-activate-additional-feature .essb-ui-switch-check').on('change', function () {
			let state = $(this).is(':checked'),
				field = $(this).data('field') || '',
				rootElement = $('.essb-activate-additional-feature-' + field),
		...
		function updateFeaturesTags() {
			$('.features-container .navigation a').each(function () {
				...
				if ($('.features-container .content .tab-' + tab + ' .single-feature').length) {
				...
				if ($('.features-container .content .tab-' + tab + ' .essb-activate-additional-feature').length) {
					total = 0;
					active = 0;
					$('.features-container .content .tab-' + tab + ' .essb-activate-additional-feature').each(function () {
						total++;
	...
	essbAdvancedOptions.save = function () {
		...
		for (var i = 0; i < paths.length; i++) {
			$(paths[i]).each(function () {
				...
				if (elementName == 'stylebuilder_css[]') {
				...
				else if (elementName == 'functions_networks[]') {
					elementName = 'functions_networks';
					options[elementName] = elementValue;
				}
				else {
	...
	$('.ao-options-btn,.ao-ui-options-btn').on('click', function (e) {
		e.preventDefault();
	...
	$('.ao-options-btn-deactivate,.ao-options-ui-btn-deactivate').on('click', function (e) {
		e.preventDefault();
	...
	$('.ao-options-btn-activate,.ao-options-ui-btn-activate').on('click', function (e) {
		e.preventDefault();
	...
	$('#essb-advanced-export-followersettings').on('click', function (e) {
		e.preventDefault();

		essbAdvancedOptions.read('export_followers_settings', {}, function (data) {
			$('#essb-exporting-followersettings-area').val(data);
			$('#essb-exporting-followersettings-area').focus();
			$('#essb-exporting-followersettings-area').select();
		});
	});	

	$('#essb-advanced-reset-followersettings').on('click', function(e) {
		e.preventDefault();
		swal({
			title: "Confirm the reset of the Followers Counter settings",
			text: "When you reset the settings it will remove all configurations you have saved for the social networks in the followers counter. This also includes the information for connection tokens or access API keys. Please confirm that the reset of the settings will continue.",
			icon: "warning",
			buttons: true,
			dangerMode: true,
		}).then((willDelete) => {
			if (willDelete) {
				essbAdvancedOptions.post('reset_followers_settings', {}, function (data) {
					$.toast({
						heading: 'The settings are successfully removed.',
						//text: 'If you are using a cache plugin, service or CDN do not forget to clear them.',
						showHideTransition: 'fade',
						icon: 'success',
						position: 'bottom-right',
	...
	$('#essb-advanced-import-followersettings').on('click', function (e) {
		e.preventDefault();

		var value = $('#essb-importing-followersettings-area').val(),
			valueObj = {};
		...
		essbAdvancedOptions.post('import_followers_settings', remotePost, function (data) {
			$.toast({
				heading: 'Settings are imported',
				showHideTransition: 'fade',
				icon: 'success',
				position: 'bottom-right',
				hideAfter: 5000
