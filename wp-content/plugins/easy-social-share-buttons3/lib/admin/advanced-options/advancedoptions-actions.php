...
class ESSBAdvancedOptions
{
	...
	public function request_parser()
	{
		...
		if ($cmd == 'export_followers_settings') {
			echo $this->export_followers_settings();
		}

		if ($cmd == 'import_followers_settings') {
			echo $this->import_followers_settings();
		}

		if ($cmd == 'reset_followers_settings') {
			$this->reset_followers_settings();
		}
	...
	public function export_followers_settings()
	{
		$output = '';

		$settings = get_option(ESSB3_OPTIONS_NAME_FANSCOUNTER);
	...
	public function reset_followers_settings() {
		delete_option(ESSB3_OPTIONS_NAME_FANSCOUNTER);
	}

	public function import_followers_settings()
	{
		$data = isset($_REQUEST['data']) ? $_REQUEST['data'] : '';
		$data = htmlspecialchars_decode($data);
		$data = stripslashes($data);

		$imported_options = json_decode($data, true);

		if (is_array($imported_options)) {
			delete_option(ESSB3_OPTIONS_NAME_FANSCOUNTER);
			update_option(ESSB3_OPTIONS_NAME_FANSCOUNTER, $imported_options, 'no');
		}
	}
	...
	public function get_options()
	{
		...
		// subscribe designs
		if (
			$current_tab == 'subscribe-design1' || $current_tab == 'subscribe-design2' ||
	...
	private function apply_functions_mode_other($current_options, $mode = '')
	{
		$default_deactivate = array(
			...
			'deactivate_module_profiles',
			'deactivate_module_natives',
			'deactivate_module_subscribe',
			'deactivate_module_facebookchat',
			'deactivate_module_skypechat',
			'deactivate_module_clicktochat',
			'deactivate_module_instagram',
	...
	private function apply_functions_mode_sharing($current_options, $mode = '')
	{
		$default_deactivate = array(
			'deactivate_ansp',
			'deactivate_ssr',
			'deactivate_fakecounters',
			'deactivate_expertcounters',
			'deactivate_ctt',
			'deactivate_module_aftershare',
			'deactivate_module_shareoptimize',
			'deactivate_module_analytics',
			...
			'deactivate_module_metrics',
			'deactivate_stylelibrary',
			'deactivate_module_translate',
			'deactivate_custombuttons',
			'deactivate_custompositions',
			'deactivate_module_conversions',
			'deactivate_method_woocommerce',
			'deactivate_method_integrations',
			'deactivate_settings_post_type',
			'deactivate_module_google_analytics',
			'deactivate_postcount',
	...
	private function apply_functions_mode($current_options, $functions_mode = '')
	{
		...
		$current_options['deactivate_postcount'] = 'false';
		$current_options['deactivate_share_counters'] = 'false';
