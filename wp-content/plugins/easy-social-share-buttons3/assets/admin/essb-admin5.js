...
jQuery(document).ready(function ($) {
	...
	var essbPreviewButtons = {
		...
		draw: function (id) {
			...
			for (var i = 0; i < drawSettings['networks'].length; i++) {
				...
				output.push('<li class="essb_item essb_link_' + networkKey + (svg_icon != '' ? ' essb_link_svg_icon' : '') + '" style="' + useDrawingStyles + '">');
				...
				output.push('<span class="essb_icon essb_svg_icon_' + networkKey + ' ' + element_icon_classes + '">' + (essb_svg_icons[networkKey] || '') + '</span>');
	...
	$("#essb_options_essb_post_og_title").on('keyup', function () {
		...
		if ($('.essb_post_og_title_len').length) {
			$('.essb_post_og_title_len').text((70 - $(this).val().length) + ' characters remain');
		}
	...
	$("#essb_options_essb_post_og_desc").on('keyup', function () {
		...
		if ($('.essb_post_og_desc_len').length) {
			$('.essb_post_og_desc_len').text((200 - $(this).val().length) + ' characters remain');
		}
	...
	function essbUpdateLiveTweetView() {
		...
		var r = [];
		r.push('<strong class="essb-title">X Message Preview</strong>');
		...
		r.push('<div class="essb-live-tweet-len essb-ui-badge essb-ui-badge-pill essb-ui-badge-lg essb-ui-badge-' + (text_tweet_version.length < 278 ? 'primary' : 'danger') + '">' + text_tweet_version.length  + '</div>');
	...
	function ssoCalculateScore() {
		...
		if (title.length < 20) {
			...
			titleStatus = 3;
			status.push('<div><span class="warning">&nbsp;</span>Title length is greater than 70 characters. Social networks may truncate it with a trailing ellipsis. </div>');
		}
		...
		if (title.length <= 200) {
			...
			descriptionStatus = 3;
			status.push('<div><span class="warning">&nbsp;</span>Description length is greater than 200 characters. Facebook may truncate it with a trailing ellipsis. </div>');
		}
		...
		if (!ssoFeaturedImage || typeof (ssoFeaturedImage) == 'undefined') ssoFeaturedImage = {};
		if (Object.keys(ssoSavedImage).length == 0) ssoSavedImage = ssoFeaturedImage;
		...
		if (ssoMediaStatus.imageSize.width == 0 || ssoMediaStatus.imageSize.height == 0) {
			if (ssoSavedImageExists || ssoSavedFeaturedImageExists) {
				imageStatus = 1;
				status.push('<div><span class="ok">&nbsp;</span>An image is provided but dimensions can\'t be validated.</div>');
			}
			else {
				imageStatus = 2;
		...
		if (ssoMediaStatus.imageSize.width >= 1150 && ssoMediaStatus.imageSize.height >= 620) {
			...
			var imageRequiredHeight = Math.floor(ssoMediaStatus.imageSize.width / 1.91),
				currentDiff = Math.abs(ssoMediaStatus.imageSize.height - imageRequiredHeight);

			var isProperAspect = currentDiff < 15 ? true : false;
	...
	if ($('#essb_options_essb_post_og_title').length) {
		$('#essb_options_essb_post_og_title').parent().append('<br/><span class="essb_post_og_title_len essb-ui-badge essb-ui-badge-pill essb-ui-badge-primary" style="margin-top: 5px;"></span>');
		$('#essb_options_essb_post_og_title').trigger('keyup');
		ssoCalculateScore();
	}

	if ($('#essb_options_essb_post_og_desc').length) {
		$('#essb_options_essb_post_og_desc').parent().append('<br/><span class="essb_post_og_desc_len essb-ui-badge essb-ui-badge-pill essb-ui-badge-primary"></span>');
